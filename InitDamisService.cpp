...
bool InitDamisService::checkForHttpError()
{
    ...
    std::vector<std::string> httpErr;

    httpErr.push_back("404 Not Found");
    httpErr.push_back("302 Found");
    ...
    {
        int errSize = httpErr.size();
        for (int i = 0; i < errSize; i++)
        {
            if(line.find(httpErr.at(i),0)!=std::string::npos) // string::npos is returned if string is not found
            {
                found=true;
                LOG (ERROR) << "HTTP stream "<< httpErr.at(i) << " found in downloaded file: " << DamisFile::getFilePath();
                ErrorResponse::setFaultDetail(std::string("File download from ") + InitDamisService::getDownloadFileURI() + std::string(" was not successful, Found HTTP stream: in data file " ) + std::string(httpErr.at(i)));
                src.close();
