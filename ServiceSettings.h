...
public:
 ...
 /*   /** \brief Decodes user file according to the user private key
      * \return bool success
      * \param uid - user ID
      * \param structre to the iledamis f
      */
	//static bool decodeUserFile(std::string uid, InitDamisService *dFile);
	...
	// static std::string pathToCertificates; /**< Path where user certificates are stored */

    // static std::string pathToUserConfig; /**< Path where user config file is stored*/
...
private:
   ...
   /* /** \brief Returns file name  of the user private key
      * \return std::string file name of the user certificate, otherwise null
      * \param uid - useID
      * \param structure of the downloaded file data
      */
	//static std::string getUserCertificate(std::string uid);
};
