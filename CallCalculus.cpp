...
void CallCalculus::run()
{
    ...
    if (ServiceSettings::runDestination == 1) //run on MII cluster
    {
     ...
     //   char * tmpNoOfProc = const_cast<char*>(std::to_string(static_cast<long long>(reqProcessors)).c_str());
       ...
       /* fileUpper <<"setenv PATH=/opt/gcc-5.2.0/bin"<< std::endl;
        fileUpper <<"setenv LD_LIBRARY_PATH=/opt/gcc-5.2.0/lib64"<< std::endl;
        fileUpper <<"setenv LIBRARY_PATH=/opt/gcc-5.2.0/lib64"<< std::endl;*/
 ...
 * Implements run of the algorithm
 */
void CallCalculus::run2()
{
    // create file for service calculations
    DamisFile *cFile = new DamisFile ("_forCalculus_");  // temporary data file that will be passed to cluster
    DamisFile *cFile2 = new DamisFile ("_forCalculus_");  // temporary data file that will be passed to cluster
    DamisFile *cFile3 = new DamisFile ("_forCalculus_");  // temporary data file that will be passed to cluster
    //DamisFile *algStat = new DamisFile ("_algRunStat_"); //temporary data file that hold data statistitics abaut algorithm run
    //variable ir ->>>>>statFile
    ...
    // save data to temporary file
    writeDataToFile(cFile->getFilePath(), prepareDataSection(serveFile->getDoubleData(), serveFile->getStringClass()), prepareAttributeSection(serveFile->getAttributeName(),serveFile->getAttributeType(), serveFile->getStringClassAttribute()));
    writeDataToFile(cFile2->getFilePath(), prepareDataSection(serveFile2->getDoubleData(), serveFile2->getStringClass()), prepareAttributeSection(serveFile2->getAttributeName(),serveFile2->getAttributeType(), serveFile2->getStringClassAttribute()));
    writeDataToFile(cFile3->getFilePath(), prepareDataSection(serveFile3->getDoubleData(), serveFile3->getStringClass()), prepareAttributeSection(serveFile3->getAttributeName(),serveFile3->getAttributeType(), serveFile3->getStringClassAttribute()));
    ...
    if (ServiceSettings::runDestination == 1) //run on MII cluster
    {
     ...
     //   char * tmpNoOfProc = const_cast<char*>(std::to_string(static_cast<long long>(reqProcessors)).c_str());
        //creating bash script with the parameters
        callCalc = ServiceSettings::pathToMPIExecutable + " -np " + std::to_string( static_cast<long long> (reqProcessors)) + " -wdir " + ServiceSettings::workingDirMPI + " " + ServiceSettings::pathToAlgorithmsExe + exeParams + " -i " +cFile->getFilePath() + " -ii " +cFile2->getFilePath() + " -iii " +cFile3->getFilePath() + " -o " + outFile->getFilePath() + " -s " + statFile->getFilePath();
